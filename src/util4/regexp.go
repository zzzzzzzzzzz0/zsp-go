...
func Regexp__(qv *zhscript.Qv___, k string, s []interface{}, s__ func(interface{}) (string, bool),
...
ret__ func(...interface{})) (no_use bool, goto1 *zhscript.Goto___) {
	...
	case "正则代":
		...
		s0, ok := s__(s[0]); if !ok {return}
		for _, s1 := range re.FindAllString(s0, -1) {
			buf, goto1, err1 = Zs2__(code, qv, s1)
			if err1 != nil {
				err__(err1)
				break
			}
